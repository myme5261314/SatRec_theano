# -*- coding: utf-8 -*-
"""
Created on Fri Mar 28 14:29:48 2014
...
from theano.tensor.utils_tutor import tile_raster_images
...
#    x = h5py.File('e:/wuhan/RawX.mat','r')
#    x_mat = numpy.asarray(x.get('rawXmem'))
#    x_mat = x_mat.T
    x = scipy.io.loadmat('e:/wuhan/pcaXscipy.mat')['X']
    y = scipy.io.loadmat('e:/wuhan/RawYscipy.mat')['Y']
    train_set_x = x[:2*2*2*2*2*5*13*19-1,:]
    test_set_x = x[2*2*2*2*2*5*13*19:,:]
    train_set_y = y[:2*2*2*2*2*5*13*19-1,:]
    test_set_y = y[2*2*2*2*2*5*13*19:,:]
...
#    test_set_x, test_set_y = train_set_x, train_set_y
    ...
    rbm = RBM(input=x, n_visible=64*64,
    ...
    for epoch in xrange(training_epochs):
        ...
        image = PIL.Image.fromarray(tile_raster_images(
                 ...
                 img_shape=(64, 64), tile_shape=(64, 64),
                 tile_spacing=(1, 1)))
